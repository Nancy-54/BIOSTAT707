...
#   set-a_long.csv and set-a_wide.csv
#   measurement range and temporal missingness EDA
#   Table 1 and outcome summary
#   missingness summary and missingness map
#
# Table 1:
...
def check_set_a():
    """Confirm that the grader-provided Set A data are in place."""
    assert OUTCOME_FILE.exists(), "data/Outcomes-a.txt is missing"
    n_files = len(list(SET_A_DIR.glob("*.txt")))
    assert n_files == EXPECTED_N, (
        f"data/set-a has {n_files} records, expected {EXPECTED_N}"
    )
...
assert len(outcomes) == EXPECTED_N, (
    f"Outcomes-a.txt has {len(outcomes)} rows, expected {EXPECTED_N}"
)
...
assert long_df["RecordID"].nunique() == EXPECTED_N, (
    f"Loaded {long_df['RecordID'].nunique()} unique records, expected {EXPECTED_N}"
)

# Required derived long table: one row per measurement, Time preserved.
long_df.to_csv(
    OUTPUT_DIR / "set-a_long.csv",
    index=False
)
...
# ============================================================
# EXPLORATORY MEASUREMENT RANGE SUMMARY
# ============================================================
eda_measurements = clinical_df.copy()
eda_measurements["Value"] = eda_measurements["Value"].replace(-1, np.nan)

measurement_range_summary = (
    eda_measurements.groupby("Parameter")["Value"]
    .agg(
        N="count",
        Min="min",
...
measurement_range_summary.to_csv(
    OUTPUT_DIR / "measurement_range_summary.csv", index=False
)

# ============================================================
# TEMPORAL MISSINGNESS FROM THE LONG TABLE
# Eight 6-hour bins across the 48-hour prediction window.
# Missing means no valid measurement for that admission,
# variable, and time interval.
# ============================================================
temporal_df = clinical_df.copy()
temporal_df["Value"] = temporal_df["Value"].replace(-1, np.nan)

time_parts = temporal_df["Time"].str.split(":", expand=True)
temporal_df["Hour"] = (
    pd.to_numeric(time_parts[0], errors="coerce")
    + pd.to_numeric(time_parts[1], errors="coerce") / 60
)
temporal_df = temporal_df[
    temporal_df["Hour"].notna()
    & (temporal_df["Hour"] >= 0)
    & (temporal_df["Hour"] <= 48)
].copy()

bin_edges = [0, 6, 12, 18, 24, 30, 36, 42, 48.000001]
bin_labels = [
    "0-6 h", "6-12 h", "12-18 h", "18-24 h",
    "24-30 h", "30-36 h", "36-42 h", "42-48 h"
]
temporal_df["Time_bin"] = pd.cut(
    temporal_df["Hour"], bins=bin_edges, labels=bin_labels,
    right=False, include_lowest=True
)

temporal_valid = temporal_df[
    temporal_df["Value"].notna() & temporal_df["Time_bin"].notna()
].copy()

temporal_observed_n = (
    temporal_valid
    .groupby(["Parameter", "Time_bin"], observed=False)["RecordID"]
    .nunique()
    .unstack(fill_value=0)
    .reindex(columns=bin_labels, fill_value=0)
)
temporal_missing_pct = (
    (EXPECTED_N - temporal_observed_n) / EXPECTED_N * 100
)

temporal_missingness = (
    temporal_missing_pct.reset_index()
    .melt(
        id_vars="Parameter",
        var_name="Time interval",
        value_name="Missing percent"
    )
)
temporal_missingness.to_csv(
    OUTPUT_DIR / "Temporal_missingness.csv", index=False
)

temporal_plot = temporal_missing_pct.loc[
    temporal_missing_pct.mean(axis=1).sort_values().index
]
plt.figure(figsize=(11, 12))
image = plt.imshow(
    temporal_plot.values, aspect="auto", interpolation="nearest"
)
plt.xlabel("Hours since ICU admission")
plt.ylabel("Variables")
plt.title("Temporal Missingness: Percent of Admissions Without a Measurement")
plt.xticks(
    range(len(bin_labels)), bin_labels, rotation=45, ha="right"
)
plt.yticks(
    range(len(temporal_plot.index)), temporal_plot.index
)
plt.colorbar(image, label="Admissions with no measurement (%)")
plt.tight_layout()
plt.savefig(
    OUTPUT_DIR / "Temporal_missingness.png",
    dpi=300,
    bbox_inches="tight"
)
plt.close()
...
# ============================================================
# CREATE REQUIRED DERIVED WIDE DATASET
#
# One row per ICU record, with count/first/last/min/max/mean
# summaries for each clinical variable.
# ============================================================

wide_source = clinical_df.copy()
wide_source["Value"] = wide_source["Value"].replace(-1, np.nan)
...
wide_summary.columns = [
    ...
    for statistic, variable in wide_summary.columns
]

wide_summary = wide_summary.reset_index()

wide_summary = wide_summary.merge(
    outcomes,
    on="RecordID",
    how="left",
    validate="one_to_one"
...
assert len(wide_summary) == EXPECTED_N, (
    f"set-a_wide has {len(wide_summary)} rows, expected {EXPECTED_N}"
)

wide_summary.to_csv(
    OUTPUT_DIR / "set-a_wide.csv",
    index=False
)
...
static_df = (
    clinical_df[
        (clinical_df["Time"] == "00:00")
        & clinical_df["Parameter"].isin(static_vars)
    ]
    .pivot_table(
        index="RecordID",
        columns="Parameter",
...
print("\n")
print(
    "CHECKPOINT 1 OUTPUTS"
)
print("==========================================")
print("1. set-a_long.csv")
print("2. set-a_wide.csv")
print("3. measurement_range_summary.csv")
print("4. Temporal_missingness.csv")
print("5. Temporal_missingness.png")
print("6. Table1.csv")
print("7. Outcome_summary.csv")
print("8. Missingness_summary.csv")
print("9. Missingness_map.png")
